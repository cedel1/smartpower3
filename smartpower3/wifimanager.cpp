...
WiFiManager::WiFiManager(WiFiUDP &udp, WiFiClient &client, ScreenSetting *screen_setting, Settings *settings)
{
	this->udp = udp;
	this->client = client;
	this->screen_setting = screen_setting;
	...
	switch(idata) {
		...
		case    '8':
			Serial.printf(
					...
					screen_setting->isLoggingEnabled() ? MSG_ON : MSG_OFF);
			switchLoggingOnOff();
			break;
		case    '9':
		case    SERIAL_CTRL_C:
