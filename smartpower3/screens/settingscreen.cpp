...
void SettingScreen::init()
{
	...
	tft->loadFont(getFont("MontserratBold20"));
...
{
	com_log_interval->clearAndDrawWithFont(getFont("MontserratBold20"), (log_value == 0) ? "OFF" : String(log_value) + " ms");
}

void SettingScreen::drawSerialBaud(uint32_t baud)
{
	com_serial_baud->clearAndDrawWithFont(getFont("MontserratBold20"), String(baud) + " bps");
}

void SettingScreen::drawSSID(String ssid)
{
	com_ssid->clearAndDrawWithFont(getFont("MontserratBold20"), ssid);
}

void SettingScreen::drawUDPIpaddr(String ipaddr)
{
	com_udp_ipaddr->clearAndDrawWithFont(getFont("MontserratBold20"), ipaddr);
}

void SettingScreen::drawUDPport(uint16_t port)
{
	com_udp_port->clearAndDrawWithFont(getFont("MontserratBold20"), (" : " + String(port)));
}
