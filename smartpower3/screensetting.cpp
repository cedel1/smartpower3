...
ScreenSetting::ScreenSetting(TFT_eSPI *tft, Settings *settings)
{
	this->tft = tft;
...
void ScreenSetting::init(uint16_t x, uint16_t y)
{
	this->x = x;
	this->y = y;
...
uint8_t ScreenSetting::_setBacklightLevelPreset(uint8_t level_preset)
{
...
uint8_t ScreenSetting::setBacklightLevelPreset(void)
{
	backlight_level_preset = backlight_level_edit;
	return _setBacklightLevelPreset(backlight_level_preset);
}

void ScreenSetting::setBacklightLevelPreset(uint8_t level_preset, bool edit)
{
...
void ScreenSetting::setBacklightLevel(uint8_t level)
{
	clampVariableToRange(0, 255, &level);
	ledcWrite(2, level);
}

void ScreenSetting::turnOffBacklight(void)
{
	this->setBacklightLevel(0);
}

void ScreenSetting::setLogInterval(uint8_t val)
{
	settings->setLogInterval(val);
	log_interval = val;
}

bool ScreenSetting::isLoggingEnabled()
{
	return settings->isLoggingEnabled();
}

void ScreenSetting::enableLogging(void)
{
	settings->setLoggingEnabled(true);
}

void ScreenSetting::disableLogging(void)
{
	settings->setLoggingEnabled(false);
}

uint8_t ScreenSetting::setLogInterval(void)
{
	settings->setLogInterval(log_interval_edit);
	return log_interval = log_interval_edit;
}

uint32_t ScreenSetting::setSerialBaud()
{
...
uint32_t ScreenSetting::setSerialBaud(uint32_t baud)
{
...
uint8_t ScreenSetting::getBacklightLevel(void)
{
	return backlight_level_preset;
}

uint8_t ScreenSetting::getLogInterval(void)
{
	return log_interval;
}

uint16_t ScreenSetting::getLogIntervalValue(void)
{
	return log_value[log_interval];
}

uint32_t ScreenSetting::getSerialBaud(void)
{
	return settings->getSerialBaudRate();
}

uint8_t ScreenSetting::getSerialBaudIndex(void)
{
	serial_baud_edit = serial_baud;
	return settings->getSerialBaudRateIndex();
}

void ScreenSetting::restoreBacklight()
{
	backlight_level_edit = backlight_level_preset;
}

void ScreenSetting::changeBacklight(uint8_t level)
{
	if (level == 255) {
...
void ScreenSetting::restoreLogIntervalValue()
{
	drawLogIntervalValue(log_value[this->log_interval]);
	log_interval_edit = this->log_interval;
}

void ScreenSetting::changeLogInterval(uint8_t log_interval)
{
...
void ScreenSetting::restoreSerialBaud()
{
	drawSerialBaud(this->serial_baud);
	serial_baud_edit = this->serial_baud;
}

void ScreenSetting::changeSerialBaud(uint8_t baud_level)
{
...
void ScreenSetting::selectBLLevel(uint16_t color)
{
...
void ScreenSetting::selectSerialLogging(uint16_t color)
{
	for (int i = 1; i < SELECTION_BORDER_WIDTH+1; i++) {
...
void ScreenSetting::selectLogInterval(uint16_t color)
{
	com_log_interval->setTextColor(color, BG_COLOR);
	com_log_interval->select();
}

void ScreenSetting::deSelectBLLevel(uint16_t color)
{
	for (int i = 1; i < SELECTION_BORDER_WIDTH+1; i++) {
		tft->drawRect(x + X_BL_LEVEL -i, y-1-i, 135+i*2, 28+i*2, color);
	}
...
void ScreenSetting::deSelectLogInterval(uint16_t color)
{
	com_log_interval->setTextColor(color, BG_COLOR);
	drawLogIntervalValue(log_value[log_interval]);
...
void ScreenSetting::deSelectSerialLogging(uint16_t color)
{
	for (int i = 1; i < SELECTION_BORDER_WIDTH+1; i++) {
		tft->drawRect(x + X_LOG_LEVEL -i-50, y-1-i + Y_SERIAL -10, 135+i*2+120, 28+i*2+40, color);
	}
...
void ScreenSetting::drawBacklightLevel(uint8_t level)
{
...
void ScreenSetting::selectSerialBaud(uint16_t color)
{
...
void ScreenSetting::deSelectSerialBaud(uint16_t color)
{
	com_serial_baud->setTextColor(color, BG_COLOR);
